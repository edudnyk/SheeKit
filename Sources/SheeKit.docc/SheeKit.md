...
SheeKit provides two modifiers for presenting the sheet, similar to SwiftUI.sheet(...):
 ...
 - controlled by the `Identifiable` item;
...
In order to customize detents, provide ``SheetProperties`` to the ``ModalPresentationStyle.pageSheet()`` or ``ModalPresentationStyle.formSheet()``.
...
### Utilise the ``UIPopoverPresentationController.adaptiveSheetPresentationController`` and customize adaptive sheet for popover
